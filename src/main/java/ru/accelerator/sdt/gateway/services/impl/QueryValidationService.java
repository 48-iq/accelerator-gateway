...
public class QueryValidationService implements ru.accelerator.sdt.gateway.services.interf.QueryValidationService {
    private final RestTemplate restTemplate;
    @Value("${hosts.services.query}")
    private String queryHost;
    @Value("${mappings.services.query.user}")
    private String userMapping;

    @Override
    public void validateUser(Integer queryId) {
        try {
            UserDetailsImpl user = (UserDetailsImpl) SecurityContextHolder.getContext().getAuthentication();
            ResponseEntity<List<Integer>> response = restTemplate.exchange(
                    queryHost + userMapping + "/" + user.getUser().getId(),
            ...
            if (!queries.contains(queryId)) {
                throw new ValidationException("this query from another user");
            }
        }
        catch (Exception e) {
